...
export default function ShippingRates() {
    ...
    const [uploadType, setUploadType] = useState<'docs' | 'non-docs'>('docs');
    ...
    }, [selectedCountry, selectedWeight, active]);
    ...
    async function handleUpload(e: React.FormEvent) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setUploadMsg('❌ ' + (_err.detail || 'Upload failed.'));
            } else {
                setUploadMsg('✅ Rates uploaded successfully!');
                await fetchShippingRates(); // Refresh rates
            }
        } catch {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#e8edf5] to-[#e6ebf3] px-4 py-10">
            <div className="w-full max-w-xl flex items-center justify-center flex-col relative">
                ...
                {/* Excel Upload UI */}
                <form onSubmit={handleUpload} className="mb-6 flex flex-col sm:flex-row items-center gap-3 w-full justify-center">
                    <div className="flex gap-2 items-center">
                        <label className="text-xs font-semibold">Type:</label>
                        <select
                            value={uploadType}
                            onChange={e => setUploadType(e.target.value as 'docs' | 'non-docs')}
                            className="px-2 py-1 border border-gray-300 rounded-full text-xs focus:outline-none"
                            disabled={uploading}
                        >
                            <option value="docs">Docs</option>
                            <option value="non-docs">PKG (Non-Docs)</option>
                        </select>
                    </div>
                    <input
                        type="file"
